...
DO NOT create duplicate config files (e.g., backend/app/core/config.py).
"""
...
class Settings(BaseSettings):
    ...
    MODEL_REGISTRY: Dict[str, Dict[str, Any]] = Field(
        default={
            ...
            "pinn": {
                ...
                "last_trained": None,
                "metrics": {"physics_residual": 0.0, "mae": 0.0, "r2": 0.0}
            }
        },
    ...
    EVENT_TYPES: Dict[str, Dict[str, Any]] = Field(
        default={
            "STATE_UPDATE": {"priority": "normal", "retention": 3600, "broadcast": True},
            "FAULT_INJECTED": {"priority": "high", "retention": 7200, "broadcast": True},
            "CONSTRAINT_VIOLATION": {"priority": "critical", "retention": 7200, "broadcast": True},
            "MODEL_PREDICTION": {"priority": "normal", "retention": 1800, "broadcast": False},
            "SIMULATION_COMPLETE": {"priority": "low", "retention": 1800, "broadcast": True}
        },
    ...
    CONTROL_LIMITS: Dict[str, Dict[str, float]] = Field(
        default={
            "battery_power": {
                "min": -100.0, "max": 100.0, "rate_limit": 50.0,
                "description": "Battery charge/discharge power"
            },
            "diesel_power": {
                "min": 0.0, "max": 200.0, "rate_limit": 20.0,
                "description": "Diesel generator output"
            },
            "curtailment": {
                "min": 0.0, "max": 100.0, "rate_limit": 10.0,
                "description": "Renewable curtailment percentage"
            },
            "v2g_power": {
                "min": 0.0, "max": 50.0, "rate_limit": 25.0,
                "description": "Vehicle-to-grid discharge power"
            },
            "load_scaling": {
                "min": 0.5, "max": 2.0, "rate_limit": 0.1,
                "description": "Load demand scaling factor"
            },
            "battery_soc": {
                "min": 0.1, "max": 0.95, "critical_low": 0.15, "critical_high": 0.90,
                "description": "Battery state of charge"
            },
            "battery_temperature": {
                "min": 0.0, "max": 50.0, "warning": 45.0, "critical": 55.0,
                "description": "Battery temperature limits"
            },
            "voltage": {
                "min": 0.95, "max": 1.05, "nominal": 1.0, "warning_low": 0.97, "warning_high": 1.03,
                "description": "Grid voltage per-unit"
            },
            "frequency": {
                "min": 59.8, "max": 60.2, "nominal": 60.0, "warning": 0.1, "critical": 0.2,
    ...
    PHYSICS_CONSTRAINTS: Dict[str, Dict[str, Any]] = Field(
        default={
            "power_balance": {
                "threshold": 1e-3,
                "severity_multipliers": {"low": 1.0, "medium": 2.0, "high": 5.0, "critical": 10.0},
            ...
            "frequency_deviation": {
                "threshold": 0.1,
                "severity_multipliers": {"low": 1.0, "medium": 2.0, "high": 3.0, "critical": 5.0},
            ...
            "swing_equation": {
                "threshold": 1e-4,
                "severity_multipliers": {"low": 1.0, "medium": 5.0, "high": 10.0, "critical": 20.0},
    ...
    ALERT_THRESHOLDS: Dict[str, Dict[str, Any]] = Field(
        default={
            "cpu_usage": {"warning": 70.0, "critical": 90.0, "unit": "percent"},
            "memory_usage": {"warning": 75.0, "critical": 90.0, "unit": "percent"},
            "disk_usage": {"warning": 80.0, "critical": 95.0, "unit": "percent"},
            "response_time": {"warning": 1000.0, "critical": 5000.0, "unit": "milliseconds"},
            "error_rate": {"warning": 0.01, "critical": 0.05, "unit": "ratio"},
            "prediction_accuracy": {"warning": 0.85, "critical": 0.70, "unit": "r_squared"},
            "physics_violation_rate": {"warning": 0.05, "critical": 0.20, "unit": "ratio"}
        },
